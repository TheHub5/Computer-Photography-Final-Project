...
def apply_vintage_filter(image):
    #vintage filter comprised of grayscale (black and white) with film grain and sepia tone
...
def pixelate_effect(image, kernel_size=7):
    ...
    # Create an empty image to store the pixelate effect
...
def display_image(np_image):
    ...
    while True:
        ...
        if event == sg.WINDOW_CLOSED or event == 'Exit':
            ...
            drawNewImage(window['-IMAGE2-'], vintage_image_data, vintage_image.shape[0]) #update IMAGE2 (2nd windonw for modified image)
